    ...
    const message = {
        ...
        state: getAllWorldActors(game).map(body => {
            const {
                ...
                angle, angularVelocity, velocityPrev, positionPrev
            } = body;
...
//             lastProcessedInput.set(client.player.body.id, input.tickNumber);
//             console.log(lastProcessedInput.get(client.player.body.id));
//         }
//     });
...
class GameServer {
    ...
    onClientConnected(socket) {
        ...
        const worldState = getAllWorldObjects(game).map(body => necromancer.stringify(body));

        this.clients.set(id, { client, player, lastProcessedInput: 0 });

        socket.on('close', this.onClientDisconnected.bind(this, id));
        socket.on('message', this.onClientMessage.bind(this));

        const playerEvent = { type: 'player-created', id, entity: necromancer.stringify(body), state: worldState };
        client.send(playerEvent);
        //     // Notify other clients before adding the body to the woruld,
        //     server.broadcast(client, { type: 'other-connected', entity: necromancer.stringify(body) });
    ...
    onClientMessage(packet) {
        ...
        switch (message.type) {
            case 'input': this.onClientInput.bind(this, message.data); break;
